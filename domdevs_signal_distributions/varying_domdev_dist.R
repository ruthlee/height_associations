varying_domdev_dist <- function ( size, population, reps, generations, epsilonrange1, epsilonrange2, avgeff, domdev, loud = TRUE, term1 = TRUE, term2 = TRUE, term3 = TRUE, term4 = TRUE, term5 = TRUE, term6 = TRUE ) {
   ...
   # }
    ...
    # now we calculate one Qx statistic, summed across the given number of populations. We use matrix method developed in "equation_verification.R"
        ...
        for ( i in 1:population ) {
            ...
            p <- afterdrift [[ i ]]
            A <- rep ( avgeff, size )
            D <- rnorm ( size, mean = domdev )

            sum_matrix <- matrix_maker ( term1, term2, term3, term4, term5, term6, size, A, D, p, epsilon )
...
y <- domdev_dist_chooseterms ( size = 1000,
                       population = 1,
                       reps = 10000,
                       generations = 10,
                       epsilonrange1 = 0.5,
...
var(y)
 ...
 hist( y, breaks = 50, main = paste ( "domdev = -0.2, avgeff = 0.5, reps = 10000, size = 1000"))
