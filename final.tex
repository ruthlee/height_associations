...
\begin{comment}
  I don't think it's quite right to say that tests for adaptation began with Wright defining $F_{ST}$. For example, Wright first wrote down the expression for $F_{ST}$ before the Hershey and Chase experiment, i.e. before it was even known that DNA was the heriditary material (and certainly long before anyone dreamed of being able to actually identify individual loci under selection.
...
\jb{So I think the first sentence of this next paragraph is not quite right. The phenotypes Spitze and others were interested in were almost certainly polygenic. The distinction here is that we have access to (some of) the underlying loci, whereas traditional $Q_{ST}$ approaches are based only on the measurement of phenotypes in common gardens. I think you could more or less just remove the first sentence of this paragraph, and then two sentences that follow probably need to get moved up into the paragraph above somewhere. Can make suggestions Tuesday...} 
...
\jb{I'm not really sure it's the increased computational abilities that are driving this. Perhaps more just the maturation of the GWAS approach.}
...
effect sizes of those loci are. \jb{In its simplest form,} GWAS works by
...
\jb{So the last sentence of the above paragraph is not right. $alpha \neq 0$ means that the allele contributes to genetic variation for the phenotype. The value of $\alpha$ alone doesn't necessarily say anything about polygenic adaptation.}


\jb{In the paragraph below here, you skip very quickly from outlining what the null is (i.e. alpha and recent allele frequency change) to saying that directional dominance violates it. I think you could include a little bit before that to more carefully outline why it is that an association between alpha and the direction of recent allele frequency change is meaningful}
...
\jb{end of below paragraph is slightly repetitive}
